...
#define EXECUTE_FUNC(FROM, FUNC)                            \
    ...
    printf("Executed successfully (%ld).\n", (FUNC)() );
...
void execute_memfd()
{
    //ssize_t len = 4096;
    int fd = memfd_create("");
    if (fd == -1) handle_error("memfd_create");
    //if (ftruncate(fd, len) == -1) handle_error("truncate");
    write(fd, code, sizeof(code)); //either write or memcpy

    char *addr = (char*)mmap(NULL, 4096, PROT_EXEC, MAP_PRIVATE, fd, 0);
    //char *addr = (char*)mmap(NULL, len, PROT_EXEC, MAP_PRIVATE, fd, 0);
    //char *addr = (char*)mmap(NULL, len, PROT_EXEC, MAP_SHARED, fd, 0);
    //char *addr = (char*)mmap(NULL, len, PROT_WRITE | PROT_EXEC, MAP_SHARED, fd, 0);
    //char *addr = (char*)mmap(NULL, len, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED, fd, 0);
    if (addr == MAP_FAILED) handle_error("mmap");

    //memcpy(addr, code, sizeof(code));
    EXECUTE_FUNC("memfd mmap (x) memory", (unsigned long (*)()) addr);
}
...
int main (int argc, char *argv[]) {
    ...
    else if (!strcmp("memfd-rwx", argv[1]))
        execute_memfd();
    else {
        help(argv[0]);
        exit(1);
...
void help(char *prog)
{
    printf("Usage: %s [data|bss|stack|stack-exec|malloc-rw|malloc-rw-x|mmap-rw|mmap-rwx|mmap-rw-x|shm-open-rwx|shmget-rw|shmget-rwx|shmget-rw-x|memfd-rwx]\n", prog);
}
