...
public:
	...
	push(T *ptr)
	{
		...
		while (__builtin_expect(tp.head >= last_tail_ + Q_SIZE, 0))
		{
            ...
            if (last_head_ == last_tail_) break; //zero break at multiple of QSIZE
	...
	pop()
	{
		...
		while (__builtin_expect(tp.tail >= last_head_, 0))
		{
            ...
            if (last_head_ == 16384) break; //last element break; multiple of QSIZE
